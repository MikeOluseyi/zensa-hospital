...
import { Search, FileText, CheckCircle2, Loader2, AlertCircle, Hash, Briefcase } from "lucide-react";
...
interface Props {
  value?: string;
  onSelect: (item: CPTItem) => void;
  placeholder?: string;
}

export default function CPTSearch({ value, onSelect, placeholder = "Search CPT code or description..." }: Props) {
  const [query, setQuery] = useState(value || "");
  const [results, setResults] = useState<CPTItem[]>([]);
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      setLoading(true);
      try {
        const res = await api.get(`/cpt/search?q=${encodeURIComponent(query)}`);
        setResults(res.data);
      } catch (err) {
        console.error("CPT search failed:", err);
        setResults([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleSelect = (item: CPTItem) => {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm font-semibold text-violet-700 bg-violet-50 px-2 py-0.5 rounded">
                      {item.code}
                    </span>
                    {item.modifier && (
                      <span className="text-xs text-slate-500">Modifier: {item.modifier}</span>
                    )}
                    {item.category && (
      ...
      {query.length >= 2 && !loading && results.length === 0 && !selected && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg p-4 text-center">
          <div className="flex flex-col items-center gap-2 text-slate-400">
            <Search size={24} />
            <p className="text-sm text-slate-500">No CPT codes found for "{query}"</p>
            <p className="text-xs text-slate-400">Try searching by code (e.g. 99213) or procedure name</p>
          </div>
        </div>
      )}
    </div>
